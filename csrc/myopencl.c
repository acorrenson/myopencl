...
// Get yourself a nice GPU!
mycl_device mycl_gpu() {
  cl_int err;
  cl_platform_id platform_id;
  cl_uint num_platforms;
  cl_uint num_devices;
  mycl_device device = malloc(sizeof(mycl_device_s));

  err = clGetPlatformIDs(1, &platform_id, &num_platforms);

  if (err != CL_SUCCESS) {
    error("[CL error] clGetPlateformIDs: %s\n", clGetErrorString(err));
  }
  success("[CL success] %u platform(s) found\n", num_platforms);

  err = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_GPU, 1, &device->id,
                       &num_devices);

  if (err != CL_SUCCESS) {
    error("[CL error] clGetDeviceIDs: %s\n", clGetErrorString(err));
  }
  success("[CL success] %u device(s) found\n", num_devices);

  return device;
}

// Print useful information about an OpenCL device
void mycl_print_device_info(mycl_device device) {
  ...
  if (err != CL_SUCCESS) {
    error("[CL error] failed to retrieve device name %s\n",
          clGetErrorString(err));
  }

  err = clGetDeviceInfo(device->id, CL_DEVICE_MAX_COMPUTE_UNITS,
                        sizeof(max_compute_units), &max_compute_units, NULL);
  if (err != CL_SUCCESS) {
    error("[CL error] failed to retrieve max compute units %s\n",
          clGetErrorString(err));
  }

  err = clGetDeviceInfo(device->id, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                        sizeof(max_group_size), &max_group_size, NULL);
  if (err != CL_SUCCESS) {
    error("[CL error] failed to retrieve max work group size %s\n",
          clGetErrorString(err));
  }
...
int main() {
  mycl_device gpu = mycl_gpu();
  mycl_print_device_info(gpu);
  return 0;
}